...
if ([string]::IsNullOrWhiteSpace($Version)) {
  $resolvedVersion = (Invoke-RestMethod -Uri "https://github.com/$repo/releases/latest/download/version.txt").Trim()
...
) { throw 'Latest release version is invalid.' }
  $tag = "v$resolvedVersion"
} else {
  $tag = if ($Version.StartsWith('v')) { $Version } else { "v$Version" }
}
$archive = "latentcolor-$tag-windows-$arch.zip"
$sums = 'checksums.txt'
